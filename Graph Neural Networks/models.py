...
class GraphSage(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, reducer='mean', 
        ...
        self.lin = nn.linear(in_channels + out_channels, in_channels, bias=False)
    ...
    def update(self, aggr_out, x):
        if self.normalize_emb:
            aggr_out = aggr_out / torch.norm(aggr_out, p='fro')
        return aggr_out
...
class GAT(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, num_heads=1, concat=True,
        ...
        self.att = nn.Parameter(torch.Tensor(size=(1, self.heads, self.out_channels)))
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        # Edge , H*C
        alpha_l = (x_i.view(-1, H, C)*self.att).sum(dim=-1)
        alpha_r = (x_j.view(-1, H, C)*self.att).sum(dim=-1)
        alpha = alpha_l + alpha_r
        alpha = F.leaky_relu(alpha, negative_slope=0.2)
        alpha = F.softmax(alpha, edge_index_i, size_i)
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)

        return x_j * alpha.view(-1, H, 1)
